...
void printCPUinfo()
{
	...
	printf("\n");
		...
		/* function  for  printing the the linux  kernal version */
void LinuxKernalVersion()
{
	...
	printf("\n");
		...
		/* function for printing the the last boot time , when the device was booted last time */
void boottime()	
{
	...
	fclose(boottime);
		...
		/* function  for  printing  the number of  interrupts handled  by  the  system  so far */
void interrupts()
{
	...
	fclose(interrupts);
		...
		/* function  for printing the total swap memory and the swap partition and their sizes */
void swapMemory()
{
